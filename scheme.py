...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(oper, rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        frame = Frame(self)       # create new Frame instance
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        ex_frame = self.env.make_child_frame(self.formals, args)
        return ex_frame
        # END PROBLEM 11
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        b_and = scheme_eval(expressions.first, env)
        if is_false_primitive(b_and):
            return False
        expressions = expressions.rest
    return b_and
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    b_or = False
    while expressions is not nil:
        expr_val = scheme_eval(expressions.first, env)
        if is_true_primitive(expr_val):
            return expr_val
        expressions = expressions.rest
    return b_or
    # END PROBLEM 12
